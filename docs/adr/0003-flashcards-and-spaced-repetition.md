...
- **Every card of the day is a different one (learner's request).** A card shown today in a direction is not offered again that day, neither later in the session nor after "Хочу ще", even when the scheduler would bring it back in a few minutes (a new word rated "Знову", "Важко" or "Добре" returns after 1, 6 or 10 minutes; "Легко" after about 8 days). Such a card is simply due again tomorrow. The cost is that the short same-day relearning steps of FSRS are skipped; the benefit is that a portion of 15 is 15 different words and the daily count is a count of different words.
- **Correction: the deck follows the lesson that is OPEN.** First the deck was "everything up to the furthest lesson reached", so opening Les 1 after finishing Les 2 still gave cards from Les 2 and 3. Now the deck is every word up to and including the open lesson, capped by how far the learner has got: open Les 1 and it is Les 1 only; open Les 5 while working on Les 5 and it is Les 1-5; peek at Les 9 while working on Les 4 and it stays Les 1-4. Opening another lesson ends a running card session, so a card of a lesson that just left the deck cannot stay on screen.
